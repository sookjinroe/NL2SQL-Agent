...
function DashboardView({ db, L, Q, counts, idx, nav }) {
  ...
  const Stat = ({ label, value, sub }) => (
    <div style={{ border: "1px solid var(--border)", borderRadius: 7, padding: "14px 16px", background: "rgba(255,255,255,0.015)" }}>
      <div style={{ ...dMono, fontSize: 12.5, letterSpacing: "0.06em", color: "var(--dim)", marginBottom: 6 }}>{label}</div>
      <div style={{ ...dMono, fontSize: 26.5, fontWeight: 700, color: "var(--text)" }}>{value}</div>
  ...
  const Bar = ({ label, a, b, color, note }) => (
    <div style={{ marginBottom: 11 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ ...dMono, fontSize: 14, color: "var(--muted)" }}>{a}/{b} · {pct(a,b)}%</span>
      </div>
      ...
      {note && <div style={{ fontSize: 12.5, color: "var(--dim)", marginTop: 3 }}>{note}</div>}
    </div>);
  const Sec = ({ title, desc, children }) => (
    <div style={{ marginBottom: 30 }}>
      <div style={{ ...dMono, fontSize: 14.5, letterSpacing: "0.06em", color: "var(--muted)", marginBottom: 4 }}>{title}</div>
      {desc && <div style={{ fontSize: 14, color: "var(--dim)", marginBottom: 12, lineHeight: 1.55 }}>{desc}</div>}
      {children}
    </div>);
  ...
  return (
    <div style={{ padding: "22px 28px", maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ marginBottom: 26 }}>
        <div style={{ fontSize: 20.5, fontWeight: 700, marginBottom: 5 }}>합성 금융 코퍼스 — 시맨틱 레이어 소비 검증</div>
          ...
          {cats.map(([k,label]) => {
            ...
            return (
              <div key={k} onClick={() => nav && nav("question", Q.find((q)=>q.cat===k)?.id)}
                ...
                <div style={{ ...dMono, fontSize: 14.5, color: "var(--text)", marginBottom: 4 }}>{n}</div>
                <div style={{ width: "100%", height: `${n/max*70}px`, background: "var(--accent)", opacity: 0.7, borderRadius: "3px 3px 0 0" }} />
                <div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 6, textAlign: "center" }}>{label}</div>
              </div>);
          })}
        </div>
      </Sec>
      ...
      <Sec title="설계된 모호성" desc="같은 표현이 여러 도메인·입도의 개념에 걸리도록 의도 설계. 에이전트가 조용히 한쪽을 고르지 않고 확인·탐색하는지 검증한다.">
        <div style={{ display: "flex", gap: 12 }}>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center" }}>
            <span onClick={() => nav && nav("collision")} style={{ ...dMono, fontSize: 14, color: "var(--lin)", cursor: "pointer",
