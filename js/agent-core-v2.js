...
2. 초안 전 확정 절차:
   ① 상대 시간(이번 달·분기·올해)은 오늘 날짜로 구체 경계를 계산해 assumptions에 적는다.
...
[제출 직전 자기 점검]
□ WHERE의 각 조건에 질문의 근거 단어가 있는가 — 없으면 그 조건을 지운다.
□ 개수·존재 질문이면 집계로 감싸 결과가 0이어도 한 행(cnt=0)이 나오는가.
□ assumptions와 SQL이 모순되지 않는가 — 모순이면 SQL을 고친다.
