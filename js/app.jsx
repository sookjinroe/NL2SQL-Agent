...
function AppShell() {
  ...
  const tabs = [["intro", "소개"], ["agent2", "NL 에이전트"], ["agent2_nosem", "NL · 레이어없음"],
                ["analyst", "분석 에이전트"], ["analyst_nosem", "분석 · 레이어없음"], ["explorer", "데이터 탐색"]];
  return (
    <div>
      ...
      <div style={{ display: tab === "agent2_nosem" ? "block" : "none" }}><window.NLScreenV2 variant="nosem" /></div>
      <div style={{ display: tab === "analyst" ? "block" : "none" }}><window.NLScreenV2 variant="analyst" /></div>
      <div style={{ display: tab === "analyst_nosem" ? "block" : "none" }}><window.NLScreenV2 variant="analyst-nosem" /></div>
