...
// NL_SYS_V2 — 디스크립션 활용형. NL_SYS 에 세 가지만 반영(나머지 동일):
//  ① get_column 설명을 '형식·코드값·뜻·분류'로 넓힘
//  ② 규칙 2의 코드 리터럴 정식 출처에 get_column.description 추가
//  ③ 규칙 6 완화 — description.confidence 가 HIGH(authored)로 독립 재확인되면 폴백이라도 HIGH 허용
const NL_SYS_V2 = NL_SYS
  .replace(
    '컬럼 상세 (id는 "테이블.컬럼").',
    '컬럼 상세 — 형식뿐 아니라 코드값·뜻(description)·분류(classification)가 함께 온다 (id는 "테이블.컬럼").'
  )
  .replace(
    'expressed_as의 value, valid_values, 또는 resolve_code로 확인된 리터럴만 쓴다. 사전 부재(error)면 코드값을 추측하지 말 것',
    'expressed_as의 value, valid_values, resolve_code로 확인된 리터럴, 또는 get_column.description에 명시된 코드값만 쓴다. resolve_code 사전이 부재(error)라도 get_column.description에 코드값이 적혀 있으면 그 값을 쓰고, 둘 다 없을 때만 코드값을 추측하지 말 것'
  )
  .replace(
    'search_columns 결과만으로 컬럼을 확정해 답할 때는 confidence를 MEDIUM 이하로 낮추고 assumptions에 폴백 사실을 적는다.',
    'search_columns 결과만으로 컬럼을 확정해 답할 때는 confidence를 MEDIUM 이하로 낮추고 assumptions에 폴백 사실을 적는다. 다만 get_column으로 그 컬럼의 description.confidence가 HIGH(source=authored)임을 독립적으로 재확인했다면, 폴백이라도 confidence를 HIGH로 올릴 수 있다.'
  );

const NL_PROMPTS = [
  { id: "baseline",    label: "현행 — 수정 전 (동결본)", system: NL_SYS },
  { id: "desc_active", label: "디스크립션 활용형 — get_column 코드·뜻 활용 + 코드 출처 인정 + 신뢰도 완화", system: NL_SYS_V2 },
];
...
return { NL_SYS, NL_SYS_V2, NL_PROMPTS, getPromptId, setPromptId, getPrompt, currentPrompt, userPrompt, MAX_OPS };
})();
