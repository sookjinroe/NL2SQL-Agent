...
2. 코드성 컬럼(_CD·_FLG)을 값으로 필터하려면 expressed_as의 value, valid_values, 또는 resolve_code로 확인된 리터럴만 쓴다. 사전 부재(error)면 코드값을 추측하지 말 것 — clarify로 값 라벨 확인을 요청하거나 cannot_answer로 부재를 보고한다.
...
5. 두 테이블을 잇는 조인은 get_join_path가 준 경로의 FK만 사용한다.
...
8. SQL은 SQLite 문법의 SELECT 단일문. 날짜는 'YYYY-MM-DD' 텍스트 비교.
...
return { NL_SYS, userPrompt, MAX_OPS };
})();
