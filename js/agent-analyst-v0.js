...
V1. 질문도 검증 대상이다. 분석은 전제 확인에서 시작한다 — 데이터가 어느 기간까지
    ...
    효과의 크기를 우연 변동(모수 대비 편차)과 견줘라. 전제가 무너지면 그 판정
    자체가 발견이다: 질문의 의도(무엇을 결정하려 했는가)에 답하라.
    ...
    for (let t = 0; t <= HARD_TURN_CAP; t++) {
      ...
      if (resp.action === "op") {
        ...
        if (isSql && r.public && r.public.ok) {
          // 팬아웃 정적 경고 (분석 계약 전용 — v2 경로 무접촉)
          const fw = (typeof ComputeCore !== "undefined" ? ComputeCore
                       : (typeof require !== "undefined" ? require("./compute-core.js") : null));
          const w = fw ? fw.fanoutCheck((resp.args || {}).sql || "", CHILD_GRAIN_TABLES) : null;
          if (w) entry.note = (entry.note ? entry.note + " / " : "") + w;
        }
        log.push(entry);
