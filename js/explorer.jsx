      ...
      <div style={{ display: "flex", gap: 2, alignItems: "center", padding: "8px 16px", borderBottom: "1px solid var(--border)" }}>
        {VIEWS.map(([k, label]) => (
          <div key={k} onClick={() => nav(k, null)}
            style={{ ...eMono, fontSize: 12.5, padding: "5px 12px", cursor: "pointer", borderRadius: 4,
        ...
        <span style={{ ...eMono, fontSize: 11.5, color: "var(--dim)", marginLeft: 12 }}>
  ...
  return (
    <div style={{ position: "relative" }}>
      <input value={q} placeholder="Term·컬럼·테이블·메트릭·질문 검색"
        ...
        style={{ ...eMono, fontSize: 12.5, width: 280, background: "rgba(0,0,0,0.3)", color: "var(--text)",
        ...
        <div style={{ position: "absolute", top: "110%", right: 0, width: 420, zIndex: 50, background: "var(--panel)",
          ...
          {results.map((r, i) => (
            <div key={i} onClick={() => go(r)}
              ...
              <span style={{ ...eMono, fontSize: 10.5, color: KIND[r.kind][1], width: 18, flexShrink: 0 }}>{KIND[r.kind][0]}</span>
              <span style={{ ...eMono, fontSize: 13, color: "var(--text)", whiteSpace: "nowrap" }}>{r.label}</span>
              <span style={{ fontSize: 12, color: "var(--dim)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.sub}</span>
            </div>))}
        </div>)}
    </div>
  );
...
function Section({ title, children }) {
  return (<div style={{ marginTop: 20 }}>
    <div style={{ ...eMono, fontSize: 12, letterSpacing: "0.08em", color: "var(--muted)", marginBottom: 9 }}>{title}</div>
    {children}</div>);
}
function Badge({ color, children, onClick }) {
  return <span onClick={onClick} style={{ ...eMono, fontSize: 10.5, color, border: `1px solid ${color}55`, borderRadius: 4,
...
function Chip({ color, children, onClick }) {
  return <span onClick={onClick} style={{ ...eMono, fontSize: 12, color: color || "var(--text)",
...
function ResultTbl({ rows }) {
  if (!rows || !rows.length) return <div style={{ ...eMono, fontSize: 12, color: "var(--dim)", marginTop: 6 }}>(0행)</div>;
  ...
  return (
    <div style={{ marginTop: 8, overflowX: "auto" }}>
      ...
      {rows.length > 8 && <div style={{ ...eMono, fontSize: 11, color: "var(--dim)", marginTop: 4 }}>… 총 {rows.length}행</div>}
    </div>);
}
...
function TableView({ db, L, idx, counts, route, nav }) {
  ...
  const left = doms.map((d) => (
    <div key={d} style={{ marginBottom: 12 }}>
      <div style={{ ...eMono, fontSize: 12, letterSpacing: "0.08em", color: DOM_COLOR[d], marginBottom: 5 }}>{d}</div>
      {L.tables.filter((t) => t.domain === d).map((t) => (
        <HoverRow key={t.name} active={sel === t.name} onClick={() => nav("table", t.name)}
          ...
          <span style={{ ...eMono, fontSize: 12.5, color: "var(--text)" }}>{t.name}</span>
          <span style={{ flex: 1 }} />
          <span style={{ ...eMono, fontSize: 11, color: "var(--dim)" }}>{(counts[t.name] || 0).toLocaleString()}</span>
        </HoverRow>))}
    </div>));
  ...
  return (
    <div style={{ maxWidth: 980 }}>
      <div style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
        <span style={{ ...eMono, fontSize: 20, fontWeight: 700 }}>{name}</span>
        <span style={{ ...eMono, fontSize: 12, color: DOM_COLOR[t.domain] }}>{t.domain}</span>
        <span style={{ fontSize: 14, color: "var(--muted)" }}>grain: {t.grain}</span>
        <span style={{ ...eMono, fontSize: 12.5, color: "var(--dim)" }}>{(counts[name] || 0).toLocaleString()}행</span>
      </div>
      <FkGraph name={name} out={out} inn={inn} nav={(n) => nav("table", n)} />
      <Section title={`컬럼 ${cols.length}개 — 레이어 오버레이 (Term 역할 뱃지 클릭 → Term 뷰)`}>
        <table style={{ ...eMono, fontSize: 12.5, borderCollapse: "collapse", width: "100%" }}>
          ...
          <tbody>{cols.map((c) => {
            ...
            return (
              <tr key={c.id} ref={isHl ? hlRef : null}
                ...
                <td style={{ padding: "4px 10px 4px 0", color: "var(--muted)", fontFamily: "var(--sans)", fontSize: 13 }}>
                  {c.description.text.split(" 값:")[0]}
                  {c.description.source === "auto" && <span style={{ ...eMono, fontSize: 10.5, color: "var(--dim)" }}> (auto)</span>}
                </td>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 16 }}>
            {dists.map((d) => (
              <div key={d.col} style={{ border: "1px solid var(--border)", borderRadius: 5, padding: "10px 13px" }}>
                <div style={{ ...eMono, fontSize: 13, color: "var(--text)", marginBottom: 8 }}>
                ...
                {d.rows.map((r) => (
                  <div key={r.v} style={{ display: "flex", gap: 8, alignItems: "center", padding: "2px 0" }}>
                    <span style={{ ...eMono, fontSize: 12, color: "var(--text)", width: 34 }}>{r.v}</span>
                    <span style={{ fontSize: 12, color: r.label ? "var(--muted)" : "var(--med)", width: 92, fontFamily: "var(--sans)" }}>
                    ...
                    <span style={{ ...eMono, fontSize: 11.5, color: "var(--dim)", width: 84, textAlign: "right" }}>
        ...
        {inn.map((e, i) => {
          ...
          return (<g key={"i" + i}>
            ...
            <text x={280} y={y + 4} textAnchor="end" onClick={() => nav(e.from)}
              style={{ fill: "var(--sig)", fontSize: 12.5, fontFamily: "var(--mono)", cursor: "pointer" }}>{e.from}</text>
            <text x={300} y={y - 5} style={{ fill: "var(--dim)", fontSize: 10, fontFamily: "var(--mono)" }}>{e.via}</text>
          </g>);
        })}
        {out.map((e, i) => {
          const y = 28 + i * rowH;
          return (<g key={"o" + i}>
            ...
            <text x={660} y={y + 4} textAnchor="start" onClick={() => nav(e.to)}
              style={{ fill: "var(--accent)", fontSize: 12.5, fontFamily: "var(--mono)", cursor: "pointer" }}>{e.to}</text>
            <text x={585} y={y - 5} style={{ fill: "var(--dim)", fontSize: 10, fontFamily: "var(--mono)" }}>{e.via}</text>
          </g>);
        })}
        <rect x={425} y={cy - 14} width={90} height={28} rx={5} fill="rgba(255,255,255,0.05)" stroke="var(--border)" />
        <text x={470} y={cy + 4} textAnchor="middle" style={{ fill: "var(--text)", fontSize: 11, fontFamily: "var(--mono)", fontWeight: 600 }}>
...
function TermView({ L, idx, route, nav }) {
  ...
  const domNav = (
    <div style={{width:140,borderRight:"1px solid var(--border)",padding:"14px 8px",overflowY:"auto",flexShrink:0}}>
      <div style={{...eMono,fontSize: 10.5,letterSpacing:"0.1em",color:"var(--dim)",marginBottom:10,paddingLeft:10}}>DOMAIN</div>
      {TERM_DOMS.map((d)=>{
        ...
        return (
          <div key={d} onClick={()=>{setActiveDom(d);setFilter("");}}
            ...
            <span style={{...eMono,fontSize: 12,color:active?color:"var(--dim)",fontWeight:active?600:400,
                          whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{label}</span>
            <span style={{...eMono,fontSize: 10.5,color:"var(--dim)",flexShrink:0}}>{domCounts[d]}</span>
          </div>);
      })}
    </div>);
  const termList = (
    <div style={{width:240,borderRight:"1px solid var(--border)",padding:"14px 8px",overflowY:"auto",flexShrink:0}}>
      <input value={filter} onChange={(e)=>setFilter(e.target.value)} placeholder="필터"
        style={{...eMono,fontSize: 12,width:"100%",background:"rgba(0,0,0,0.3)",color:"var(--text)",
                border:"1px solid var(--border)",borderRadius:4,padding:"4px 8px",marginBottom:8}} />
      {listTerms.length===0&&<div style={{...eMono,fontSize: 12,color:"var(--dim)",padding:"4px 6px"}}>(없음)</div>}
      {listTerms.map((t)=>{
        ...
        return (
          <HoverRow key={t.name} active={active} onClick={()=>nav("term",t.name)}
            ...
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              <span style={{fontSize: 14,color:active?"var(--text)":"var(--muted)",flex:1,lineHeight:1.4}}>{t.name}</span>
              {hasCol&&<span style={{...eMono,fontSize: 10,color:"var(--low)"}}>⚠</span>}
              {t.family&&<span style={{...eMono,fontSize: 9.5,color:"var(--med)"}}>{t.family.split("_")[0]}</span>}
            </div>
            <div style={{...eMono,fontSize: 10.5,color:"var(--dim)"}}>{(t.links||[]).length} links</div>
          </HoverRow>);
      })}
    </div>);
  return (
  ...
  return (
    <div style={{ maxWidth: 880 }}>
      <div style={{ display: "flex", gap: 10, alignItems: "baseline", flexWrap: "wrap" }}>
        <span style={{ fontSize: 21, fontWeight: 700 }}>{t.name}</span>
        <span style={{ ...eMono, fontSize: 12, color: DOM_COLOR[t.domain] || "var(--dim)" }}>{t.domain || "공통"}</span>
      ...
      <div style={{ fontSize: 15, color: "var(--muted)", margin: "10px 0 0", lineHeight: 1.6 }}>{t.definition}</div>
        ...
        <Section title="실현(links) — 역할별 · 자산 클릭 시 해당 뷰로">
          {roles.map((r) => (
            <div key={r} style={{ display: "flex", gap: 10, alignItems: "baseline", padding: "4px 0" }}>
              <span style={{ ...eMono, fontSize: 12, color: ROLE_COLOR[r], width: 110, flexShrink: 0 }}>{r}</span>
              <div>
...
function MetricView({ L, idx, route, nav }) {
  ...
  const left = L.metrics.map((m) => (
    <HoverRow key={m.id} active={sel === m.id} onClick={() => nav("metric", m.id)}
      ...
      <div style={{ ...eMono, fontSize: 11, color: "var(--dim)" }}>{m.id}</div>
    </HoverRow>));
  const m = L.metrics.find((x) => x.id === sel) || L.metrics[0];
  const right = (
    <div style={{ maxWidth: 820 }}>
      <div style={{ display: "flex", gap: 10, alignItems: "baseline" }}>
        <span style={{ fontSize: 21, fontWeight: 700 }}>{m.name}</span>
        <span style={{ ...eMono, fontSize: 12.5, color: "var(--dim)" }}>{m.id}</span>
      </div>
      <Section title="정의식 (expr)">
        <pre style={{ ...eMono, fontSize: 14, background: "rgba(0,0,0,0.25)", border: "1px solid var(--border)",
      ...
      <Section title="기준 필터 (base_filters) — 정본과 소박한 재계산을 가르는 지점">
        {(m.base_filters || []).length
          ...
          : <span style={{ ...eMono, fontSize: 12.5, color: "var(--dim)" }}>(없음)</span>}
      </Section>
      {m.note && <Section title="주석"><div style={{ fontSize: 14.5, color: "var(--muted)" }}>{m.note}</div></Section>}
      <Section title="grain"><span style={{ ...eMono, fontSize: 13, color: "var(--text)" }}>{m.grain}</span></Section>
      <Section title="이 지표로 측정되는 Term (measured_by 역방향)">
        ...
        {!(idx.metricTerms[m.id] || []).length && <span style={{ ...eMono, fontSize: 12.5, color: "var(--low)" }}>⚠ 고아 메트릭 — 도달 경로 없음</span>}
      </Section>
    </div>);
    ...
    return (
      <div style={{ border: `1px solid ${isSel ? "var(--sig)" : "var(--border)"}`, borderRadius: 6, padding: "11px 14px",
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 8 }}>
          <span style={{ ...eMono, fontSize: 14, color: "var(--text)" }}>{e.col}</span>
          <span style={{ ...eMono, fontSize: 11, color: DOM_COLOR[e.domain] || "var(--dim)" }}>{e.domain}</span>
          <span style={{ flex: 1 }} />
          <span onClick={() => nav("table", e.table, e.id)} style={{ ...eMono, fontSize: 11, color: "var(--dim)", cursor: "pointer" }}>{e.table} →</span>
        </div>
        {e.desc && <div style={{ fontSize: 12, color: "var(--muted)", marginBottom: 9, lineHeight: 1.5 }}>{e.desc}</div>}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
          {Object.entries(e.dict).map(([code, label]) => (
            <span key={code} style={{ ...eMono, fontSize: 12, border: "1px solid var(--border)", borderRadius: 4, padding: "2px 8px" }}>
  ...
  return (
    <div style={{ padding: "18px 26px", maxWidth: 1040 }}>
      <div style={{ border: "1px solid var(--border)", borderRadius: 7, padding: "14px 18px", marginBottom: 24,
        ...
        <div style={{ ...eMono, fontSize: 13, color: "var(--sig)", marginBottom: 8 }}>코드사전이란?</div>
        <div style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.75 }}>
          코드성 컬럼(<span style={{ ...eMono, fontSize: 13 }}>_CD</span>로 끝나는 컬럼)에 저장된 <b style={{ color: "var(--text)", fontWeight: 500 }}>코드값과 그 의미의 매핑</b>이다.
          예를 들어 <span style={{ ...eMono, fontSize: 13, color: "var(--sig)" }}>REGION_CD='SE'</span>가 "서울"을 뜻한다는 정보로,
          에이전트가 "서울 지역" 같은 자연어를 올바른 코드값으로 변환(<span style={{ ...eMono, fontSize: 13 }}>resolve_code</span>)할 때 쓰인다.
          프로그래밍 코드가 아니라 <b style={{ color: "var(--text)", fontWeight: 500 }}>값 사전</b>이다.
        </div>
        <div style={{ fontSize: 14.5, color: "var(--muted)", lineHeight: 1.75, marginTop: 8 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {missing.map((e) => (
            <div key={e.id} onClick={() => nav("table", e.table, e.id)}
              ...
              <div style={{ ...eMono, fontSize: 13, color: "var(--low)" }}>{e.col}</div>
              <div style={{ ...eMono, fontSize: 11, color: "var(--dim)", marginTop: 2 }}>{e.table} · {e.domain}</div>
            </div>))}
        </div>
      </Section>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {identifier.map((e) => (
            <span key={e.id} onClick={() => nav("table", e.table, e.id)}
              style={{ ...eMono, fontSize: 12, color: "var(--dim)", border: "1px solid var(--border)", borderRadius: 4,
  ...
  return (
    <div style={{ padding: "18px 26px", maxWidth: 1040 }}>
      <div style={{ border: "1px solid var(--border)", borderRadius: 7, padding: "14px 18px", marginBottom: 24,
        ...
        <div style={{ ...eMono, fontSize: 13, color: "var(--low)", marginBottom: 8 }}>충돌 지도란?</div>
        <div style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.75 }}>
        ...
        <div style={{ fontSize: 15, color: "var(--muted)", lineHeight: 1.75, marginTop: 8 }}>
          ...
          <span style={{ ...eMono, fontSize: 12.5, color: "var(--low)", marginLeft: 6 }}>⚠</span> 표시가 있는 말은 에이전트가 반드시 확인 질문을 해야 한다(D8 채점 정책).
        </div>
      </div>
          ...
          {Object.entries(idx.families).map(([f, terms]) => {
            ...
            return (
              <div key={f} style={{ border: `1px solid ${sel===f?"var(--med)":"var(--border)"}`, borderRadius: 6, padding: "11px 14px" }}>
                <div style={{ ...eMono, fontSize: 13, color: "var(--med)", marginBottom: 5 }}>{FAM_LABEL[f]||f}</div>
                <div style={{ fontSize: 12.5, color: "var(--low)", marginBottom: 4 }}>충돌 어휘: {clash[0]}</div>
                <div style={{ fontSize: 12.5, color: "var(--dim)", marginBottom: 9, lineHeight: 1.55 }}>{clash[1]}</div>
      ...
      <Section title={`동의어 충돌 표 — 같은 말이 2개 이상 Term에 닿는 표면형 ${idx.collisions.length}개`}>
        ...
        {idx.collisions.map((c) => (
          <div key={c.word} style={{ display: "flex", gap: 12, alignItems: "center", padding: "7px 12px", borderRadius: 5,
            ...
            <span style={{ ...eMono, fontSize: 14.5, color: "var(--low)", width: 112, flexShrink: 0 }}>"{c.word}"</span>
            <span style={{ ...eMono, fontSize: 11, color: "var(--dim)", width: 30, flexShrink: 0 }}>→</span>
            <div style={{ display: "flex", flexWrap: "wrap" }}>
              {c.terms.map((tn) => {
  ...
  const left = cats.map((cat) => (
    <div key={cat} style={{ marginBottom: 11 }}>
      <div style={{ marginBottom: 6 }}>
          <span style={{ ...eMono, fontSize: 12, letterSpacing: "0.08em", color: "var(--muted)" }}>{ECAT[cat].label.toUpperCase()}</span>
          <div style={{ fontSize: 11.5, color: "var(--dim)", marginTop: 2, lineHeight: 1.5 }}>{ECAT[cat].desc}</div>
        </div>
      {Q.filter((q) => q.cat === cat).map((q) => (
        <HoverRow key={q.id} active={sel === q.id} onClick={() => nav("question", q.id)}
          ...
          <div style={{ lineHeight: 1.5 }}>
            <span style={{ ...eMono, fontSize: 11, color: "var(--dim)", marginRight: 5 }}>{q.id}</span>
            <span style={{ fontSize: 12.5, color: "var(--text)" }}>{q.text}</span>
...
function RunnableSql({ db, sql, label }) {
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: 5, padding: "9px 12px", marginBottom: 9 }}>
      {label && <div style={{ ...eMono, fontSize: 12, color: "var(--med)", marginBottom: 6 }}>{label}</div>}
      <pre style={{ ...eMono, fontSize: 12.5, whiteSpace: "pre-wrap", margin: 0, color: "var(--text)" }}>{sql}</pre>
      <div style={{ marginTop: 7 }}>
        <span onClick={() => { try { setErr(null); setRows(window.Scorer.runSql(db, sql)); } catch (e) { setErr(String(e.message || e)); } }}
          style={{ ...eMono, fontSize: 12, color: "var(--high)", border: "1px solid var(--high)66", borderRadius: 4,
                   padding: "2px 10px", cursor: "pointer" }}>실행</span>
        {err && <span style={{ ...eMono, fontSize: 12, color: "var(--low)", marginLeft: 8 }}>{err}</span>}
      </div>
    ...
    <span ref={ref} style={{ display: "inline-block", flexShrink: 0 }}
      ...
      {pos && tip && (
        <span style={{ position: "fixed", left: pos.x, top: pos.y,
          ...
          <span style={{ ...eMono, fontSize: 11, color, display: "block", marginBottom: 3 }}>{m}</span>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ ...eMono, fontSize: 11, letterSpacing: "0.1em", color: "var(--med)", marginBottom: 5 }}>
          체크포인트 · {q.id} · {catLabel}
        </div>
        ...
        <div style={{ border: "1px solid var(--border)", borderLeft: "3px solid var(--med)", borderRadius: 5,
          ...
          {cp.must && (
            <div style={{ display: "flex", gap: 10, marginBottom: 7 }}>
              <span style={{ ...eMono, fontSize: 11.5, color: "var(--high)", flexShrink: 0, width: 100 }}>해야 할 것</span>
              <span style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.6 }}>{cp.must}</span>
            </div>)}
          {cp.watch && (
            <div style={{ display: "flex", gap: 10, marginBottom: 7 }}>
              <span style={{ ...eMono, fontSize: 11.5, color: "var(--sig)", flexShrink: 0, width: 100 }}>트레이스에서</span>
              <span style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.6 }}>{cp.watch}</span>
            </div>)}
          {cp.trap && (
            <div style={{ display: "flex", gap: 10 }}>
              <span style={{ ...eMono, fontSize: 11.5, color: "var(--low)", flexShrink: 0, width: 100 }}>함정·주의</span>
              <span style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6 }}>{cp.trap}</span>
            </div>)}
        </div>)}
      ...
      {q.mode === "missing" && (
        <Section title="기대 행동">
          <div style={{ fontSize: 14, color: "var(--muted)", lineHeight: 1.65, marginBottom: 10 }}>{q.golden.expected_behavior}</div>
